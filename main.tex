...
\usepackage[left=20mm, right=30mm, top=30mm, bottom=35mm]{geometry} % Margins
\usepackage{fancyhdr}
...
\newlist{odkazy}{enumerate}{1}
\setlist[odkazy,1]{label=[\arabic*]}
\lstset{
    language=Python,
    basicstyle=\ttfamily,
    numbers=none,
...
\addbibresource{bib/SampleBib.bib}

% Příkazy pro název a autora
\newcommand{\mytitle}{Sazba technických dokumentů – semestrální projekt}
\newcommand{\myauthor}{Irzhytskyi Dmytro, IRZ0006}
...
% Přidání kontitulů a čísla stránky
...
\label{lst:example}
\begin{lstlisting}[language=Python, caption={Ukázkový výpis zdrojového kódu}]
...
\subsection{Vyprávějí drak dělat tvrdí vrak úrovně} \label{subsection:3.1} \zlabel{sec:3.1-page}
...
Vtéto sekci zkusíme ocitovat různé zdroje, jde například o \cite{sigfridsson, augustine, kastenholz, kullback, maron}. Dále vyzkoušíme křížové odkazy ta tabulku \hyperref[table:tabulka]{1}, na obrázek \hyperref[fig:obrazek]{1}, na rovnici \hyperref[eq:rovnice]{(1)}, na výpis zdrojového kódu \hyperref[lst:example]{1}, na kapitolu \hyperref[subsection:3.1]{3.1} na straně \zpageref{sec:3.1-page} a na přílohu \hyperref[section:priloha]{A}.

\nocite{*}
\printbibliography[title={Odkazy}] 

% Změna zobrazení sekce a návrat ke standardnímu zobrazení (nastaveno pouze pro tuto sekci)
...
\renewcommand{\thesection}{\Alph{section}} 
\section{Příloha} \label{section:priloha}
